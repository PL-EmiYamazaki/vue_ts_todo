...
import { InAddCardEvent, InMoveCardEvent } from "@/components/List.vue";
...
export default class App extends Vue {
  ...
  listCreatedCount = createInitialLists().length;
  cardCreatedCount = 4;
  draggedList: IList | null = null;
  draggedCardData: InCardData | null = null;
  ...
  addCard({ listId, text }: InAddCardEvent): void {
  ...
  removeCard({ listId, cardId }: InRemoveCardEvent): void {
    const list = this.lists.find(list => list.id === listId);
    if (list === undefined) return;
  ...
  moveCard({ listId, cardId, event }: InMoveCardEvent): void {
    if(
